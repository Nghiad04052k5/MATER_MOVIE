...
import { Monitor, Armchair, CheckCircle2, QrCode, Popcorn, Plus, Minus } from 'lucide-react'
...
const COMBOS = [
  { id: 'solo', name: 'Combo Solo', desc: '1 Bắp lớn + 1 Nước ngọt lớn', price: 79000 },
  { id: 'couple', name: 'Combo Couple', desc: '1 Bắp lớn + 2 Nước ngọt lớn', price: 109000 },
  { id: 'family', name: 'Combo Family', desc: '2 Bắp lớn + 4 Nước ngọt lớn', price: 189000 },
]
...
export default function ClientSeatSelector({ showtime, seats, bookedSeatIds }: { showtime: Showtime, seats: Seat[], bookedSeatIds: string[] }) {
  ...
  const [step, setStep] = useState<'SEATS' | 'COMBOS' | 'PAYMENT' | 'SUCCESS'>('SEATS')
  ...
  const combosTotal = Object.entries(selectedCombos).reduce((acc, [comboId, qty]) => {
     const combo = COMBOS.find(c => c.id === comboId)
     return acc + (combo ? combo.price * qty : 0)
  }, 0)

  const totalAmount = seatsTotal + combosTotal
  ...
  if (step === 'PAYMENT' && currentTicketId) {
     const seatNames = selectedSeats.map(s => `${s.seat_row}${s.seat_col}`).join(', ')
     const hasCombo = Object.values(selectedCombos).some(qty => qty > 0)
     const transferContent = `N THERA VE ${seatNames}${hasCombo ? ' COMBO' : ''}`
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 items-start">
       {/* PHẦN HIỂN THỊ ĐỘNG THEO STEP BÊN TRÁI */}
       {step === 'SEATS' && (
           <div className="flex-1 w-full bg-slate-900/50 p-6 md:p-12 rounded-3xl border border-slate-800 backdrop-blur-sm overflow-x-auto overflow-y-hidden shadow-2xl animate-in fade-in duration-300">
              ...
              <div className="flex justify-center items-center mt-12 gap-6 md:gap-10 border-t border-slate-800 pt-8 flex-wrap">
       ...
       {step === 'COMBOS' && (
           <div className="flex-1 w-full bg-slate-900/50 p-6 md:p-12 rounded-3xl border border-slate-800 backdrop-blur-sm shadow-2xl animate-in slide-in-from-right-8 duration-300">
               <h2 className="text-2xl font-black text-white flex items-center gap-2 mb-8"><Popcorn className="text-[#00f2fe]" /> Thêm Bắp Nước Khuyến Mãi</h2>
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                   {COMBOS.map(combo => (
                       <div key={combo.id} className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6 flex flex-col items-center text-center transition-all hover:border-[#00f2fe]/50 hover:shadow-[0_0_30px_rgba(0,242,254,0.1)] group">
                           <div className="w-24 h-24 bg-slate-900 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                               <Popcorn size={48} className="text-[#4facfe] group-hover:text-[#00f2fe] transition-colors" />
                           </div>
                           <h3 className="text-xl font-bold text-white mb-2">{combo.name}</h3>
                           <p className="text-sm text-slate-400 mb-6 flex-1">{combo.desc}</p>
                           <p className="text-[#00f2fe] font-bold text-xl font-mono mb-4">{combo.price.toLocaleString()}đ</p>
                           
                           <div className="flex items-center gap-4 bg-slate-900 rounded-xl p-1 border border-slate-800 w-full justify-between">
                               <button 
                                   onClick={() => setSelectedCombos(prev => ({ ...prev, [combo.id]: Math.max(0, (prev[combo.id] || 0) - 1) }))}
                               ...
                               <span className="w-6 font-bold text-white text-lg">{selectedCombos[combo.id] || 0}</span>
                               <button 
                                   onClick={() => setSelectedCombos(prev => ({ ...prev, [combo.id]: (prev[combo.id] || 0) + 1 }))}
                                   className="w-10 h-10 rounded-lg bg-[#00f2fe]/20 flex items-center justify-center text-[#00f2fe] hover:bg-[#00f2fe] hover:text-black transition-colors"
                               ><Plus size={18} /></button>
                           </div>
                       </div>
                   ))}
       ...
       {/* THANH TOÁN (CART) BÊN PHẢI (LUÔN HIỂN THỊ TRỪ KHI SUCCESS HOẶC PAYMENT) */}
          ...
          <div className="flex-1 space-y-4">
             {selectedSeats.length === 0 ? (
                ...
                <div className="space-y-3">
                   ...
                   {/* Render Combos */}
                   {Object.entries(selectedCombos).map(([comboId, qty]) => {
                      if (qty === 0) return null;
                      const combo = COMBOS.find(c => c.id === comboId);
                      if (!combo) return null;
                      return (
                         <div key={comboId} className="flex justify-between items-center bg-slate-800/50 p-3 rounded-xl border border-slate-700/50 animate-in slide-in-from-left-2">
                            <div>
                               <span className="font-bold text-white tracking-widest">{combo.name} <span className="text-xs text-slate-400 bg-slate-800 px-1 py-0.5 rounded ml-1">x{qty}</span></span>
                            </div>
                            <span className="font-mono text-sm text-[#00f2fe]">{(combo.price * qty).toLocaleString()}đ</span>
                         </div>
                      )
                   })}
                </div>
          ...
          <div className="mt-8 border-t border-slate-800 pt-6">
             ...
             {step === 'SEATS' && (
                 <button 
                    ...
                    className="w-full py-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-black uppercase tracking-widest transition-all disabled:opacity-20 border border-slate-700"
                 >
             ...
             {step === 'COMBOS' && (
                 <div className="flex gap-2">
                     <button 
                        ...
                        className="px-4 py-4 rounded-xl border border-slate-700 text-slate-400 font-bold hover:bg-slate-800 hover:text-white transition-colors"
                     >
                        Quay lại
                     </button>
                     <button 
                        ...
                        disabled={loading}
                        className="flex-1 py-4 rounded-xl bg-gradient-to-r from-[#00f2fe] to-[#4facfe] hover:from-[#4facfe] hover:to-[#00f2fe] text-slate-950 font-black uppercase tracking-widest transition-all disabled:opacity-20 shadow-[0_0_20px_rgba(0,242,254,0.3)] hover:shadow-[0_0_30px_rgba(0,242,254,0.5)]"
                     >
                        {loading ? 'ĐANG GIỮ GHẾ...' : 'THANH TOÁN'}
                     </button>
                 </div>
             )}
          </div>
