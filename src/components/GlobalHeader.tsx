...
export default function GlobalHeader({ user }: { user: User | null }) {
  ...
  // Ẩn hoàn toàn Header công khai nếu đang ở khu vực quản trị Admin hoặc Staff
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/10 px-8 py-4 flex items-center justify-between">
      ...
      <nav className="hidden md:flex gap-8 text-sm font-medium text-slate-300">
        ...
        <Link href="/showtimes" className="hover:text-[#00f2fe] transition-colors">LỊCH CHIẾU</Link>
        <Link href="/cinemas" className="hover:text-[#00f2fe] transition-colors">RẠP PHIM</Link>
        <Link href="/offers" className="hover:text-[#00f2fe] transition-colors">ƯU ĐÃI</Link>
      </nav>
